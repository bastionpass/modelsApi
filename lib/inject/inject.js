    ...
    Object.defineProperty(target, propertyName, {
        get() {
            if (!target.injectionNamespace) {
                throw new internals_1.CoreError(`Please decorate a class with @inject before decorating a property` +
...
function getInjected(namespaceOrObject, injectionConstructor) {
    ...
    if (swagger_ts_types_1.isObject(namespaceOrObject)) {
        if (!namespaceOrObject.injectionNamespace) {
            throw new internals_1.CoreError(`Cannot determine namespace for ${namespaceOrObject.constructor.name}.`);
        }
...
function checkInjectionType(target, injectedType) {
    if (!(injectedType && 'constructor' in injectedType)) {
        throw new internals_1.CoreError(`Injected type should be a class, but \'${injectedType}\' on ${target.name}. ` +
            'Check if you have a circular injection.');
    }
    if (/\{\s*\[native code\]\s*\}/.test(`${injectedType}`)) {
        throw new internals_1.CoreError(`Cannot inject native objects ${injectedType} on ${target}`);
    }
    return true;
}
