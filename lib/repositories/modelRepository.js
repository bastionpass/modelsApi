...
const customRepository_1 = require("./customRepository");
const inject_1 = require("../inject/inject");
const log_1 = require("../log/log");
const swagger_ts_types_1 = require("swagger-ts-types");
function isObservableModel(arg) {
    return swagger_ts_types_1.isObject(arg) && arg._loadState instanceof loadState_1.LoadState && arg._modelType;
}
exports.isObservableModel = isObservableModel;
const defaultList = 'all';
let ModelRepository = class ModelRepository extends customRepository_1.CustomRepository {
    constructor(modelType, modelMetadata, isModel, mainRepository) {
        ...
        this.loadList = (list) => {
            ...
            list.loadState = loadState_1.LoadState.pending();
            const fetchPromise = this.fetchList(list.name).then((rawModels) => {
                ...
                list.loadState = new loadState_1.ErrorState(error);
                this.fetchPromises.delete(list);
                if (!(error instanceof coreError_1.CoreError)) {
                    throw error;
                }
            });
     ...
     */
    getModel(id) {
        return new ObservableOptionalModel_1.ObservableOptionalModel(this.getRawModel(id), this.modelType, this.mainRepository);
    }
    /**
    ...
    createOrUpdate(model, saveModel) {
        ...
        return apiPromise.then((responseModel) => {
            ...
            model._loadState = new loadState_1.ErrorState(error);
            throw error;
        });
    }
    deleteModel(model) {
        ...
        if (realModel) {
            realModel._loadState = loadState_1.LoadState.pending();
        }
        this.deleteOne(model).then(() => {
            if (realModel) {
                realModel._loadState = loadState_1.LoadState.done();
            ...
            if (realModel) {
                model._loadState = new loadState_1.ErrorState(apiError);
            }
            throw apiError;
        });
    }
    ...
    getFilteredList(filter) {
        ...
        const filteredList = new modelList_1.FilteredModelListImpl(listName, all, filter);
        return filteredList;
    }
    /**
     * internal use only
     ...
     */
    createEmptyModel(id) {
        return mobx_1.observable.object(Object.assign({}, this.modelMetadata.emptyModel, { id, _loadState: loadState_1.LoadState.none(), _modelType: this.modelType }));
    }
    /**
    ...
    createEmptyList(name, filter) {
        return new modelList_1.ModelListImpl(name, this.loadList, filter);
    }
    /**
    ...
    loadModel(model) {
        ...
        model._loadState = loadState_1.LoadState.pending();
        ...
        if (fetchPromise) {
            ...
            list.loadList().then(() => {
                ...
                if (model._loadState.isPending()) {
                    model._loadState = new loadState_1.ErrorState(new coreError_1.CoreError(`Model ${this.modelType} ${model.id} was not found on backend in default list`));
                }
            }).catch((apiError) => {
                model._loadState = new loadState_1.ErrorState(apiError);
            });
    ...
    consumeModels(rawModels, implList) {
        ...
        for (const index in rawModels) {
            ...
            if (swagger_ts_types_1.isModelWithId(rawModel)) {
                ...
                if (normalizingError) {
                    ...
                    model._loadState = new loadState_1.ErrorState(normalizingError);
                }
                else {
                    models.push(model);
                    model._loadState = loadState_1.LoadState.done();
                }
            }
            else {
                invalidModels.push(rawModel);
        ...
        list.loadState = invalidModels.length
            ? new loadState_1.ErrorState(new coreError_1.CoreError(`${invalidModels.length} invalid models came from backed. ${JSON.stringify(rawModels)}`))
            : loadState_1.LoadState.done();
    ...
    consumeModel(model, rawModel) {
        if (swagger_ts_types_1.isModelWithId(rawModel)) {
            ...
            model._loadState = new loadState_1.ErrorState(new coreError_1.CoreError(`Denormalizing error: model has no id ${JSON.stringify(rawModel)}`));
        }
    }
    getModelType() {
...
ModelRepository = __decorate([
    inject_1.inject,
    __metadata("design:paramtypes", [typeof (_a = typeof ModelTypes !== "undefined" && ModelTypes) === "function" && _a || Object, Object, Function, mainRepository_1.MainRepository])
], ModelRepository);
